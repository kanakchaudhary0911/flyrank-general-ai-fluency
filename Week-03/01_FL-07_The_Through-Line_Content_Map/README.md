# FL-07: The Through-Line — Map Content & CTAs

**Track:** General AI Fluency
**Week:** 3
**Phase:** Foundations
**Workload:** 2h

## Objective

The purpose of this assignment is to map the portfolio content before building it. This includes defining the one-line claim, target hiring person, primary action, page structure, case-study placement, calls to action, and the proof that still needs to be gathered.

This assignment builds on FL-03: **What Are You Proving**.

---

## 1. Portfolio Positioning

### Primary Technical Claim

I design data models that anticipate future features and make the engineering tradeoffs behind them explicit.

### Target Hiring Person

**Backend Engineering Lead** evaluating entry-level backend talent for long-term engineering potential.

### Primary Action

Review the **SpellForge/Reikein** GitHub repository and the engineering reasoning documented around its database design. If the approach is a fit, reach out.

The portfolio follows an **evidence-first** approach: show credible technical proof before asking for contact.

---

## 2. One-Line Claim

> **I design data models that anticipate future features and make the engineering tradeoffs behind them explicit.**

This is the portfolio-facing version of the positioning developed in FL-03. It keeps the claim specific to data modeling and engineering reasoning rather than using a generic developer statement.

---

## 3. Content Map

### Home

**Sections in order:**

1. Hero — one-line claim
2. Short proof/context
3. Featured case study — SpellForge/Reikein
4. Selected additional projects
5. Technical capabilities
6. Final CTA

**Primary CTA:** Review the SpellForge/Reikein case study

**Secondary CTA:** View selected projects

---

### Projects / Case Studies

**Sections in order:**

1. Page introduction
2. Featured case study — SpellForge/Reikein
3. Case study — AI Internship Landing Page
4. Case study — Student Performance Analysis
5. Additional selected projects
6. Final CTA

#### Featured Case Study Structure

1. Problem
2. What I built
3. Architecture / system context
4. Database design
5. XP History design decision
6. Engineering trade-off
7. Why this approach
8. Technical stack
9. Repository / demo
10. What I would improve next

**Primary CTA:** Review the SpellForge/Reikein repository and design reasoning

**Final CTA:** Get in touch

---

### About

**Sections in order:**

1. Short introduction
2. What I focus on
3. Backend engineering interests
4. Technical skills
5. How I approach engineering decisions
6. Internship / project experience
7. Resume
8. CTA

**Primary CTA:** View my technical work

**Final CTA:** Get in touch

---

### Contact

**Sections in order:**

1. Short invitation
2. Contact information
3. Contact form
4. LinkedIn
5. GitHub
6. Resume
7. Final CTA

**Primary CTA:** Get in touch

---

## 4. Case-Study Priority

### 1. SpellForge / Reikein

**Lead case study**

This is the strongest case for the primary technical claim because the database design demonstrates deliberate engineering reasoning, especially the XP History design decision.

### 2. AI Internship Landing Page

**Supporting case study**

Demonstrates practical web-development work and the ability to build and ship a working project.

### 3. Student Performance Analysis

**Supporting case study**

Demonstrates Python, data analysis, and working with structured data.

### 4. Additional Projects

Used as secondary evidence after the strongest cases.

---

## 5. CTA Ladder

The portfolio follows an evidence-first path:

**Home → Featured Proof → Case Study → Repository / Technical Reasoning → Contact → Reach Out**

The portfolio should not push contact before showing credible evidence.

The case study and repository provide the proof first; outreach is the final action.

---

## 6. Still Need to Gather

The evidence list is intentionally honest so the portfolio build is not blocked by missing proof.

### SpellForge / Reikein — Highest Priority

* [ ] Final database schema
* [ ] Final ER diagram
* [ ] README section explaining the XP History decision
* [ ] Clear explanation of the normalization vs. query-simplicity trade-off
* [ ] Clean database/design screenshots
* [ ] Verified GitHub repository URL
* [ ] Verified live demo URL, if available
* [ ] Final case-study screenshots

### Other Portfolio Projects

* [ ] AI Internship Landing Page screenshots
* [ ] AI Internship Landing Page live demo URL
* [ ] Verified repository URL
* [ ] Student Performance Analysis screenshots
* [ ] Verified repository URL
* [ ] Screenshots for any additional selected projects

### Portfolio Assets

* [ ] Final resume PDF
* [ ] Verified LinkedIn URL
* [ ] Verified GitHub profile URL
* [ ] Final contact details
* [ ] Professional profile photo, if used

### Evidence Rules

* [ ] Do not invent performance numbers.
* [ ] Do not invent testimonials.
* [ ] Do not present unfinished work as finished.
* [ ] Prefer real screenshots and real repository/demo links over generated substitutes.

---

## 7. FL-03 → FL-07 Connection

FL-03 answered:

> **What am I proving?**

FL-07 answers:

> **Where and how will I prove it?**

The same technical claim, target hiring person, and evidence-first action should remain consistent throughout the portfolio.

---

## Final Deliverable

This assignment contains:

* One-line claim
* Target hiring person
* Primary action
* Content map
* Page sections in order
* Case-study placement
* Page-level CTAs
* CTA ladder
* Still-need-to-gather list

The goal is to have a clear content structure before moving to the visual identity and image-curation assignments in Week 3.
