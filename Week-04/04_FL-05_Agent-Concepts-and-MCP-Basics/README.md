...
For the practical component, I connected Google Drive to Claude and used the integration to search and read files from Google Drive. The tasks demonstrated how an AI client can access external information through a connected tool rather than relying only on normal chat context.

---

## Objectives

- Understand the difference between AI workflows and agents.
- Identify the characteristics of a structured AI workflow.
- Understand the basic concepts of Model Context Protocol (MCP).
- Learn the role of MCP tools, resources, and prompts.
- Connect an external service to an AI client.
- Use a connected service to search and retrieve external information.
- Identify how a fixed workflow could be evolved toward a more autonomous agent.
...
## 2. Classification of FL-13 Workflow

My FL-13 **Draft → Critique → Revise** system is a **workflow**, not an agent.

The process has clearly defined stages:
...
For the practical component, I connected **Google Drive to Claude**.
...
The permission screen and resulting file content demonstrated that the external file was accessed through the connector.
...
The system could evaluate the critique after each revision and decide whether another revision is necessary.

If important information is missing, it could identify what additional information is needed before continuing.

If the output already satisfies the quality criteria, it could stop instead of running unnecessary steps.
...
- Distinguish between a predefined workflow and a more autonomous agent.
- Explain the role of tools, resources, and prompts in MCP.
- Connect an external service to an AI client.
- Use a connected Google Drive integration to search and retrieve files.
- Understand how external information can be used by an AI system.
- Identify the additional autonomy required to evolve a workflow toward an agent.
- Recognize the continued importance of human verification and judgment.
...
```text
FL-05_Agent-Concepts-and-MCP-Basics
│
├── README.md
└── screenshots
    ├── 01_mcp_connector_setup.png
    ├── 02_mcp_task_01_google_drive_search.png
    ├── 03_mcp_task_02_permission.png
    ├── 04_mcp_task_02_result.png
    ├── 05_mcp_task_03_tool_call.png
    └── 06_mcp_task_03_result.png
```
